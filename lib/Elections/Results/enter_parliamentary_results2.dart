...
Future<ElectionParliamentaryCandidatesModel> get_election_parliamentary_candidates(constituency_id) async {
  ...
  final response = await http.get(
    Uri.parse(hostName + "elections/get-all-election-parliamentary-candidates/?constituency_id=${constituency_id}"),
  ...
  if (response.statusCode == 200) {
    ...
    return ElectionParliamentaryCandidatesModel.fromJson(jsonDecode(response.body));
...
class _EnterParliamentaryResultScreen2State extends State<EnterParliamentaryResultScreen2> {
  List<TextEditingController> textControllers = [];
  GlobalKey<FormState> formKey = GlobalKey<FormState>();
  late Future<ElectionParliamentaryCandidatesModel> _futureGetParliamentaryCandidate;

  @override
  void initState() {
    super.initState();
    _futureGetParliamentaryCandidate = get_election_parliamentary_candidates(widget.data["constituency_id"]);
    _futureGetParliamentaryCandidate.then((value) {
      var data = value.data;
      if (data != null) {
        setState(() {
          textControllers = List.generate(data.length, (_) => TextEditingController());
        });
      }
    });
  }
                                      ...
                                      children: [
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            image: DecorationImage(
                                              image: NetworkImage(hostNameMedia +
                                                  _parliamentaryCandidates[index].candidate!.photo!
                                                      .toString()),
                                              fit: BoxFit.cover,
                                            ),
                                          ),
                                        ...
                                        Container(
                                          ...
                                          decoration: BoxDecoration(
                                            ...
                                            image: DecorationImage(
                                              image: NetworkImage(hostNameMedia +
                                                  _parliamentaryCandidates[index].candidate!.party!
                                                      .partyLogo!
                                                      .toString()),
                                              fit: BoxFit.cover,
                                            ),
                            ...
                            onTap: () async {
                              if (formKey.currentState!.validate()) {
                                ...
                                for (int i = 0; i < textControllers.length; i++) {
                                  results.add({
                                    'election_parl_id': _parliamentaryCandidates[i].electionParlId.toString(),
                                  ...
                                  if (response.statusCode == 200) {
                                    ...
                                    showDialog(
                                      ...
                                      builder: (BuildContext context) {
                                        return AlertDialog(
                                          title: Text("Error"),
                                          content: Text("Failed to submit data. Please try again.", style: TextStyle(color: Colors.black),),
                                          actions: <Widget>[
                                            TextButton(
                                              child: Text("OK"),
                                              onPressed: () {
